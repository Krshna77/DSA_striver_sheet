...
bool SearchInRotatedAndSOrted(vector<int> &nums, int target)
{
    int n = nums.size();
    int low = 0, high = n - 1;
    while (low <= high)
    {
        ...
        if (nums[mid] == target)
            return   true ;// if you find the target then return it from there only
...
int main()
{
    vector<int> arr = {7, 8, 9, 1, 2, 3, 3, 3, 6};
    int target = 3;
